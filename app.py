...
.riskbar-fill { height: 100%; border-radius: 8px; transition: width 0.6s ease; }
...
if "total_churn" not in st.session_state:
    st.session_state.total_churn = 0
...
def preprocess(inp):
    ...
    for col, val in [
        ('MultipleLines', inp['multiple_lines']),   ('InternetService', inp['internet']),
        ('OnlineSecurity', inp['online_security']), ('OnlineBackup', inp['online_backup']),
        ('DeviceProtection', inp['device_protect']),('TechSupport', inp['tech_support']),
        ('StreamingTV', inp['streaming_tv']),       ('StreamingMovies', inp['streaming_movies']),
        ...
        if key in dummies:
            dummies[key] = 1
    return pd.DataFrame([{**base, **dummies}])
...
with col_left:
    ...
    # Feature importance (static based on LR knowledge of this dataset)
    st.markdown('<div class="panel">', unsafe_allow_html=True)
    ...
    fig_factors = go.Figure(go.Bar(
        x=factor_df["Score"], y=factor_df["Factor"],
        orientation="h",
...
with col_right:
    ...
    rows_html = "".join([
        ...
        f"<span style='font-weight:500;font-size:0.82rem;'>{v}</span>"
        f"</div>"
    ...
    if st.session_state.total_analyzed > 0:
        fig_donut = go.Figure(go.Pie(
            ...
            values=[st.session_state.total_churn, safe_count],
            hole=0.65,
            marker_colors=["#ff3d6b", "#00e096"],
            textinfo="none",
        ))
        fig_donut.update_layout(
            ...
            annotations=[{"text": f"{churn_rate:.0f}%<br><span style='font-size:10px'>churn</span>",
                           "x": 0.5, "y": 0.5, "showarrow": False,
                           "font": {"size": 22, "color": "#ff3d6b", "family": "Syne"}}],
        )
...
# ── Bottom: Probability trend over session ───────────────────────────
if len(st.session_state.history) >= 2:
    ...
    fig_trend.add_hline(y=50, line_dash="dash", line_color="#ffb300", line_width=1,
    ...
    fig_trend.update_layout(
        ...
        yaxis={"gridcolor": "#1e2d45", "tickfont": {"color": "#5a6a82"},
